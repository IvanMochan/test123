        ...
        if response.status_code == requests.codes['ok']:
            ...
            if opinions_count:
                ...
                while(url):
                    ...
                    with open(f"app/data/opinions/{product_id}.json", "w", encoding="UTF-8") as jf:  # "jf" to jeden plik z tymi wyscrappowanymi danymi 
                    ...
                    with open(f"app/data/products/{product_id}.json", "w", encoding="UTF-8") as jf:  # "jf" to jeden plik z tymi wyscrappowanymi danymi 
...
def product(product_id):
    ...
    return render_template("product.html.jinja", product_id=product_id, product = product)
...
def download_csv(product_id):
    ...
    return send_file(f"data/opinions/{product_id}.json", "text/csv", download_name=f"{product_id}.csv",as_attachment = True)
